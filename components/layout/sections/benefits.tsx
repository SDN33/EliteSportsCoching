...
export const BenefitsSection = () => {
  return (
    <section id="presentation" className="relative py-14 sm:py-20 md:py-32 overflow-hidden">
      ...
      <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-white/5 to-transparent rounded-full blur-3xl -z-10" />
          ...
          <div data-reveal="left" className="flex flex-col gap-6 sm:gap-8 items-center lg:items-start">
            ...
            <div className="space-y-4 sm:space-y-6">
              <p className="text-base sm:text-lg leading-relaxed text-muted-foreground text-center lg:text-left">
                ...
                préparation de gestion de vie privé et accompagnement humain global.
              </p>
              <p className="text-base sm:text-lg leading-relaxed text-muted-foreground text-center lg:text-left">
                Approche unique : diagnostic précis → plan d&apos;action clair → suivi intensif.
                Chaque détail compte, chaque étape se mesure.
              </p>
              <div className="text-sm sm:text-base leading-relaxed text-white/80 font-semibold p-3 sm:p-4 rounded-lg border border-white/20 bg-white/5 flex items-start gap-3">
          ...
          <div className="grid gap-5 grid-cols-1">
            {pillars.map(({ icon: Icon, title, description }, index) => (
              <article
                ...
                className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-white/5 to-white/[0.02] backdrop-blur-lg p-4 sm:p-6 transition-all duration-500 hover:border-white/30 hover:-translate-y-2 hover:shadow-2xl hover:shadow-white/10"
              >
                {/* Animated background gradient on hover */}
                <div className="absolute inset-0 bg-white/0 group-hover:bg-white/5 transition-all duration-500 -z-10" />

                <div className="flex items-start gap-4">
                  <div className="mt-1 inline-flex rounded-lg border border-white/20 bg-white/10 p-3 text-white/80 group-hover:border-white/40 group-hover:bg-white/15 transition-all duration-300">
